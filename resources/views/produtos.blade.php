        ...
        <table class="table table-striped table-bordered">
            ...
            <tbody id="tbody">
                ...
                @foreach ($produtos as $produto)
                    <tr>
                        {{-- Status do produto --}}
                        <td class="text-center align-middle"><input type="checkbox" name="status_produto"
                                class="form-check-input" {{ $produto->status_produto === 's' ? 'checked' : '' }}
                                value="{{ ($produto->status_produto = 's') ? 's' : 'n' }}"></td>
                        {{-- Nome do produto --}}
                        <td class="text-center align-middle"><input type="text" name="nome_produto"
                                value="{{ $produto->nome_produto }}" class="form-control"></td>
                        {{-- Preço --}}
                        <td class="text-center align-middle"><input type="text" name="preco_produto"
                                value="{{ number_format($produto->preco_produto, 2, ',', '.') }}" class="form-control"
                                onkeyup="formatarPreco(this)"></td>
                        {{-- Categoria --}}
                        <td class="text-center align-middle">
                            <select name="categoria_produto" id="categoria" class="form-select">
                                @foreach ($categorias as $categoria)
                                    <option value="{{ $categoria->id }}"
                                        {{ $categoria->id === $produto->id_categoria ? 'selected' : '' }}>
                        ...
                        {{-- Medida --}}
                        <td class="text-center align-middle">
                            <input name="medida_produto" type="text" value="{{ $produto->medida_produto }}"
                                class="form-control">
                        </td>
                        <td class="text-center align-middle">
                            <div class="btn-group">
